...
def build_settled_v3_accuracy_lab_report(
    ...
    *,
    ...
    if undated_events_present:
        leakage_checks["chronological_boundaries_non_decreasing"] = False
...
def _lead_days_bucket(lead_time_days: int | None) -> str:
...
def _cohort_metrics(rows: list[dict[str, Any]]) -> dict[str, Any]:
    market_rows = [r for r in rows if r.get("market_prob") is not None]
    blend_rows = _with_prob(
        rows,
        "frozen_v31_blend_prob",
        lambda r: (
            0.5 * float(r["model_prob"]) + 0.5 * float(r["market_prob"])
            if r.get("model_prob") is not None and r.get("market_prob") is not None
            else None
        ),
    )
    blend_covered = [
        ...
        if r.get("frozen_v31_blend_prob") is not None and r.get("actual") is not None
    ]
    return {
        "v3": _metrics(rows, "model_prob"),
        "kalshi_baseline": {
            "coverage_n": len(market_rows),
            "metrics": _metrics(market_rows, "market_prob"),
        },
        "frozen_v31_50_50_blend": {
            "coverage_n": len(blend_covered),
            "metrics": _metrics(blend_rows, "frozen_v31_blend_prob"),
        },
    }
...
def _counterfactual_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "all_same_day_contracts": _cohort_metrics(rows),
        "threshold_only_same_day": _cohort_metrics(threshold_rows),
        "range_only_same_day": _cohort_metrics(range_rows),
        "exclude_range_counterfactual": {
            "retained_contract_pct": (
                round(len(threshold_rows) / len(rows) * 100, 2) if rows else None
            ),
            "dropped_range_contracts": len(range_rows),
            "retained_threshold_contracts": len(threshold_rows),
            "all_same_day_n": len(rows),
        },
    }


def build_same_day_counterfactual_report(
    cohort_rows: list[dict[str, Any]],
    *,
    as_of: datetime | None = None,
) -> dict[str, Any]:
    """
    Build a same-day threshold-vs-range counterfactual report from frozen cohort rows.

    This function accepts rows in the settled_v3_main_cohort.csv / main_cohort JSON
    shape from the accuracy lab. If exact same-day fields are unavailable, it fails
    closed with an explicit blocker while still reporting the coarser lead-time data
    ...
    for row in normalized_rows:
        lead_distribution_rows.append({
            ...
            "coarse_lead_bucket": str(row.get("lead_time_bucket", "unknown")),
        })

    exact_counts: dict[tuple[str, str], int] = defaultdict(int)
    coarse_counts: dict[tuple[str, str], int] = defaultdict(int)
    for row in lead_distribution_rows:
    ...
    def _dist(
        ...
        for evidence_class, total in sorted(total_map.items()):
            ...
            for bucket in labels:
                n = counts.get((evidence_class, bucket), 0)
                rows_out.append({
                    "bucket": bucket,
                    "n": n,
                    "pct_of_evidence_class": round(n / total * 100, 2) if total else 0.0,
                })
            out[evidence_class] = rows_out
        return out
    ...
    if not exact_same_day_supported:
        ...
        proxy_research = [
            ...
            if row.get("eligibility_class") == "RESEARCH_ONLY"
        ]
        report["status"] = "BLOCKED_MISSING_EXACT_SAME_DAY_FIELD"
        report["blocker"] = {
            "missing_fields": ["lead_time_days"],
            "why_blocked": (
                "The committed settled_v3_main_cohort artifact preserves only the "
                "coarse 0-1d / 2-3d / 4-7d lead bucket, so it cannot separate exact "
                "same-day rows from next-day rows inside 0-1d."
            ),
            "minimum_additional_input": (
                "Re-run from the complete settled V3 export with lead_time_days "
    ...
    same_day_research = [
        row for row in exact_by_evidence.get("RESEARCH_ONLY", [])
        if row.get("lead_time_days") == 0
    ]
    threshold_same_day = [
        row for row in same_day_research if row.get("contract_type") == "threshold"
    ]

    report["status"] = "ok"
    report["same_day_by_evidence_class"] = {
        evidence_class: _cohort_metrics(
            [row for row in rows if row.get("lead_time_days") == 0]
        )
        for evidence_class, rows in sorted(exact_by_evidence.items())
    }
    report["research_same_day_counterfactual"] = _counterfactual_summary(same_day_research)
    report["research_same_day_breakdowns"] = {
        "by_contract_type": _breakdown(same_day_research, lambda r: str(r["contract_type"])),
        "by_city_station": _breakdown(same_day_research, lambda r: str(r["station_bucket"])),
        "by_probability_bucket": _breakdown(same_day_research, lambda r: str(r["probability_bucket"])),
        "by_market_price_bucket": _breakdown(same_day_research, lambda r: str(r["market_bucket"])),
        "by_model_disagreement": _breakdown(same_day_research, lambda r: str(r["disagreement_bucket"])),
    }
    report["research_threshold_only_same_day_breakdowns"] = {
        "by_city_station": _breakdown(threshold_same_day, lambda r: str(r["station_bucket"])),
        "by_probability_bucket": _breakdown(threshold_same_day, lambda r: str(r["probability_bucket"])),
        "by_model_disagreement": _breakdown(threshold_same_day, lambda r: str(r["disagreement_bucket"])),
    }
    return report
