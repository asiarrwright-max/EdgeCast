...
- No V2.1 row is modified.  No V2.2 paper trade exists yet (paper trading off).
"""
...
async def get_strategy_comparison(
    ...
    strategies = {
        ...
        "v22": _strategy_summary(
            ...
            description="Corrected-bias version — same as V2.1 but bias sign fixed (mu += bias). "
                        "Paper trading disabled; predictions only.",
        ),
    ...
    for ticker in all_tickers:
        ...
        # Derive city/variable from whichever strategy has the trade
        ...
        # Agreement: all present versions chose same direction
        ...
        # Derived deltas (only when all three present)
    ...
    smoke_test = {
        ...
        "note": (
            "V2.2 paper trading is DISABLED. No V2.2 paper trades exist yet. "
            "The corrected bias formula activates only when a forecast_error_stats "
            "bucket crosses MIN_SAMPLE = 30 observations — currently not met. "
            "Until that threshold is crossed, V2.1 and V2.2 probabilities are "
            "numerically identical and prob_delta_pp = 0 for all markets. "
